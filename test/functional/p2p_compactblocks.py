...
# TestNode: A peer we use to send messages to redecoind, and store responses.
...
class CompactBlocksTest(redecoinTestFramework):
    ...
    # This test actually causes redecoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that redecoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for redecoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # redecoind will not send blocktxn responses for blocks whose height is
