...
The RPM spec file provided here is for redecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://redecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[redecoin git repository](https://github.com/redecoinProject/redecoin) but are not part of
...
Currently only `Source10` is used, for the example `redecoin.conf` file.
...
in the redecoin git repository. At present only an SVG version of the redecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the redecoin
...
needed when 0.12.1 is released, a proper fix is already in the redecoin git
master branch.
...
idea to build redecoin Core against a static version of that library compiled
...
The `redecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `redecoin-server` package. At this point, most RPM based
...
The `redecoin-server` package should have SELinux support. How to properly do
...
redecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/redecoin/). He has been
packaging redecoin for Fedora at least since 2012.
...
4. I renamed the `redecoin` package that contains the Qt GUI to `redecoin-core` as
that appears to be how the general population refers to it, in contrast to
`redecoin-xt` or `redecoin-classic`. I wanted to make sure the general population
