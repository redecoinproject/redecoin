...
sudo find ${DISTNAME}/bin -type f -executable -exec ../redecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../redecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/redecoin-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer redecoin-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer redecoin-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer redecoin-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/redecoin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/redecoinproject/redecoin
mkdir ~/redecoin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder redecoin/depends/SDKs
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/redecoin-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer redecoin-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
