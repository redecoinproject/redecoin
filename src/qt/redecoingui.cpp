...
const std::string redecoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString redecoinGUI::DEFAULT_WALLET = "~Default";

redecoinGUI::redecoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void redecoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a redecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and redecoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to REDE addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your redecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified redecoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a redecoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible redecoin command-line options").arg(tr(PACKAGE_NAME)));
...
void redecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void redecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** REDE START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/redecointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* redecoinLabelLayout = new QVBoxLayout(toolbarWidget);
        redecoinLabelLayout->addWidget(label);
        redecoinLabelLayout->addWidget(toolbar);
        redecoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        redecoinLabelLayout->addStretch(1);
...
void redecoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool redecoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool redecoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void redecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void redecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void redecoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void redecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void redecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void redecoinGUI::optionsClicked()
{
...
void redecoinGUI::showDebugWindow()
{
...
void redecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void redecoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void redecoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void redecoinGUI::openClicked()
{
...
void redecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void redecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void redecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void redecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void redecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void redecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** REDE START */
void redecoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void redecoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void redecoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void redecoinGUI::gotoRestrictedAssetsPage()
{
...
void redecoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to redecoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void redecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void redecoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void redecoinGUI::updateHeadersSyncProgressLabel()
{
...
void redecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void redecoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("redecoin"); // default title
...
void redecoinGUI::changeEvent(QEvent *e)
{
...
void redecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void redecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void redecoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "REDE")
        msg += tr("Amount: %1\n").arg(redecoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(redecoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void redecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void redecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool redecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool redecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void redecoinGUI::setHDStatus(int hdEnabled)
{
...
void redecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void redecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void redecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void redecoinGUI::detectShutdown()
{
...
void redecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void redecoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void redecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(redecoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void redecoinGUI::subscribeToCoreSignals()
{
...
void redecoinGUI::unsubscribeFromCoreSignals()
{
...
void redecoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void redecoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<redecoinUnits::Unit> units = redecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const redecoinUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(redecoinUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(redecoinUnits::name(unit)));
        #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(redecoinUnits::name(u)), this);
...
{
    setText(redecoinUnits::name(newUnits));
}
...
void redecoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=REDEBTC"));
