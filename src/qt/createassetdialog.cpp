...
void CreateAssetDialog::setUpValues()
{
    ...
    QStringList list;
    list.append(tr("Main Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::ROOT)) + ")");
    list.append(tr("Sub Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB)) + ")");
    list.append(tr("Unique Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::UNIQUE)) + ")");
    list.append(tr("Messaging Channel Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::MSGCHANNEL)) + ")");
    list.append(tr("Qualifier Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::QUALIFIER)) + ")");
    list.append(tr("Sub Qualifier Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB_QUALIFIER)) + ")");
    list.append(tr("Restricted Asset") + " (" + redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::RESTRICTED)) + ")");
...
void CreateAssetDialog::CheckFormState()
{
    ...
    if (!(ui->addressText->text().isEmpty() || IsValidDestination(dest)) && assetNameValid) {
        ...
        showMessage(tr("Warning: Invalid redecoin address"));
        return;
    }
...
void CreateAssetDialog::onCreateAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetBurnAmount(type), 8)) + " REDE";
    amount.append("</b>");
    ...
    if(nFeeRequired > 0)
    {
        ...
        questionString.append(redecoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void CreateAssetDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void CreateAssetDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid redecoin address"));
        }
        else // Valid address
        {
...
void CreateAssetDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void CreateAssetDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
                redecoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetRequiredFee(1000)) + "/kB")
        );
}
