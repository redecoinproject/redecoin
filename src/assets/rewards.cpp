...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for REDE
    ...
    if (p_rewardSnapshot.strDistributionAsset != "REDE") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is REDE with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using REDE differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "REDE") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse redecoin address (already validated during ownership snapshot creation)
